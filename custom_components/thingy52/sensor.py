...
from homeassistant.const import (ATTR_FRIENDLY_NAME, CONF_MAC, CONF_SENSORS,
                                 TEMP_CELSIUS)
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(
            CONF_REFRESH_INT, default=DEFAULT_REFRESH_INTERVAL
        ): cv.time_period,
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    sensors = []
    _LOGGER.debug(
        "#[THINGYSENSOR]: Connecting to Thingy %s with address %s",
        friendly_name,
        mac_address[-6:],
    )
    try:
        ...
        _LOGGER.error(
            "#[THINGYSENSOR]: Unable to connect to Thingy (%s): %s",
            friendly_name,
            str(e),
        )
    ...
    for sensorname in conf_sensors:
        ...
        sensors.append(
            Thingy52Sensor(
                thingy,
                sensorname,
                SENSOR_TYPES[sensorname][0],
                friendly_name,
                SENSOR_TYPES[sensorname][1],
                SENSOR_TYPES[sensorname][2],
                mac_address,
            )
        )
...
class Thingy52Sensor(Entity):
    ...
    def __init__(
        self, thingy, name, sensor_name, friendly_name, unit_measurement, icon, mac
    ):
